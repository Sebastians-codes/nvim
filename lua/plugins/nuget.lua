...
return {
  ...
  config = function()
    ...
    -- Find the closest .sln file from current directory
    function M.find_closest_sln()
      ...
      while true do
        ...
        for _, f in ipairs(files) do table.insert(up_sln, f) end
        local parent = vim.fn.fnamemodify(path, ':h')
        if parent == path then break end
        path = parent
      end
      local all_sln = {}
      for _, s in ipairs(sub_sln) do table.insert(all_sln, s) end
      for _, s in ipairs(up_sln) do table.insert(all_sln, s) end
      if #all_sln == 0 then
        return nil
      end
      -- Remove duplicates
      local unique = {}
      for _, s in ipairs(all_sln) do unique[s] = true end
      local unique_list = {}
      for s in pairs(unique) do table.insert(unique_list, s) end
    ...
    -- Parse .sln file to extract .csproj paths
    function M.parse_sln_projects(sln_path)
      if not sln_path then return {} end
    ...
    function M.search_packages(query)
      ...
      if not search_url then return {} end
      local curl = require('plenary.curl')
